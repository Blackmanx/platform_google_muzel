 ...
 * newlines in printed strings to spaces.
 */
...
DEFINE_BPF_PROG("iter/dmabuf", AID_ROOT, AID_SYSTEM, iter_dmabuf)
(struct bpf_iter__dmabuf* ctx) {
    ...
    if (pname) {
        if (bpf_probe_read_kernel(name, sizeof(name), pname)) return 1;
